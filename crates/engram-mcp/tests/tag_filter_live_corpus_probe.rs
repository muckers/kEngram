...
//! `search_thoughts(scope="engram.m3.dogfood", tag_filter={"kind":"task"})`
...
//!     cargo test -p engram-mcp --test dogfood_tag_filter_probe -- --ignored --nocapture

use engram_embed::FakeEmbedder;
use engram_mcp::{SearchRequest, search_thoughts};
use sqlx::PgPool;
...
async fn dogfood_kind_task_filter_returns_zero_on_observation_only_corpus() {
    ...
    let baseline = search_thoughts(
        ...
        None,
        SearchRequest {
            query: "engram".to_string(),
            scope: Some(engram_core::Scope::new("engram.m3.dogfood").unwrap()),
            scope_prefix: None,
            limit: Some(50),
