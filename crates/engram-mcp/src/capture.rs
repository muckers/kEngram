...
//! `embedding_status: "pending"`. The `engram worker` process drains both
...
/// until a tagger is configured and the operator runs `engram tag --rerun`.
pub async fn capture(
    ...
    let (inserted, is_new) = engram_storage::insert_thought(
        pool,
    ...
    if is_new {
        ...
        if let Some(tagger_id) = tagger_model_id {
            engram_storage::enqueue_tag_job(pool, inserted.id, tagger_id).await?;
        }
    }
...
mod tests {
    ...
    async fn writes_thought_and_enqueues_returns_pending(pool: PgPool) {
        ...
        let tag_jobs = engram_storage::fetch_pending_tag_jobs(&pool, 10)
            .await
            .unwrap();
    ...
    async fn enqueues_embedding_and_tag_jobs_on_new_insert(pool: PgPool) {
        ...
        let pending_embeds = engram_storage::count_pending(&pool).await.unwrap();
        assert_eq!(pending_embeds, 1);

        let pending_tag_jobs = engram_storage::fetch_pending_tag_jobs(&pool, 10)
            .await
            .unwrap();
