//! `engram backup` / `engram restore` — portable, manifest-validated
//! machine-to-machine corpus migration. M7 operator subcommands.
//!
//! Both commands shell out to system `pg_dump` / `pg_restore`; engram does
//! not reimplement them. The value-add over a bare `pg_dump | pg_restore`
//! pipeline is the `manifest.json` sidecar (engram version, schema head,
...
pub async fn run_backup(
    ...
    // Default output path: ./engram-backup-<RFC3339-compact>.tar.gz in CWD.
    let output_path = match to {
        ...
        None => {
            let stamp: String = manifest
                ...
                .map(|c| if c == ':' || c == '.' { '-' } else { c })
                .collect();
            PathBuf::from(format!("./engram-backup-{stamp}.tar.gz"))
        }
    };
    ...
    // tempdir).
    let tmp = std::env::temp_dir().join(format!("engram-backup-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&tmp).with_context(|| format!("creating tempdir {}", tmp.display()))?;

    let result = (async {
        // 1. pg_dump.
        let dump_path = tmp.join("engram.dump");
...
pub async fn run_restore(
    ...
    // 1. Extract tarball.
    let tmp = std::env::temp_dir().join(format!("engram-restore-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&tmp).with_context(|| format!("creating tempdir {}", tmp.display()))?;
    ...
    let result = (async {
        ...
        if !manifest_path.exists() {
            bail!(
                "archive missing manifest.json — not a valid engram backup: {}",
                from.display()
            );
        }
        let dump_path = tmp.join("engram.dump");
        if !dump_path.exists() {
            bail!(
                "archive missing engram.dump — not a valid engram backup: {}",
                from.display()
            );
        }
        ...
        if manifest.manifest_version > MANIFEST_VERSION {
            bail!(
                "manifest version {} is newer than this binary understands ({}); \
                 upgrade engram on the target machine first",
        ...
        if existing_thoughts > 0 && !force {
            println!("engram restore — DRY RUN (target has existing data)");
            println!();
                ...
                SchemaCompat::TargetEmpty { .. } => {
                    bail!(
                        "target database has no schema applied. Run \
                         `sqlx migrate run` (or `engram migrate`) on the target first."
                    );
                }
            ...
            if manifest.embedder.model_id != config.embedder.model_id {
                eprintln!(
                    "warning: backup embedder is `{}` ({}d); target is configured \
                     ...
                     consider `engram embed-backfill` afterward.",
        ...
        println!("engram restore — proceeding");
        println!();
        print_manifest_summary(&manifest);
        println!();
        ...
        println!();
        println!(
            "restored from {} (backup created {}). Run `engram stats` to verify.",
...
async fn build_manifest(
    ...
    let audit_entries = engram_storage::query_migration_audit(pool, None, 1000)
        .await
    ...
    let stats = engram_storage::corpus_stats(pool, None)
        .await
...
fn print_manifest_summary(m: &BackupManifest) {
    ...
    println!("  engram:    v{}", m.engram_version);
...
mod tests {
    ...
    fn sample_manifest() -> BackupManifest {
        BackupManifest {
            manifest_version: 1,
            engram_version: "0.1.0".into(),
            created_at: OffsetDateTime::from_unix_timestamp(1_700_000_000).unwrap(),
            source_database: "postgres://engram:<redacted>@localhost:5432/engram".into(),
            schema: SchemaInfo {
                head_version: 11,
