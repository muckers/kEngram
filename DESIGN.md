...
- A *write* web UI. A **read-only** web UI shipped at M8 (search/visualize, `docs/milestones/m8-human-read-surface.md`); `psql` remains the write/admin interface.
...
- **2026-06-30** — **M8 opened: read-only human web surface (reverses the v0 "no web UI" non-goal).** kEngram gets a browser surface for a human to search and visualize the corpus, served by the same `kengram` binary that serves `/mcp`. The reversal is narrow and recorded in the out-of-scope list above + `CLAUDE.md` + `docs/milestones/m7-operational-maturity.md`: the UI is **read-only**; `psql` stays the write/admin interface and `/mcp` stays the agent interface. Architecture (decided with Ron): a new `kengram-web` library crate (9th workspace member; `kengram-cli` stays the only binary) mounts read-only `/api/*` JSON routes + server-rendered HTML onto the existing axum router behind a `[web].enabled` flag (default false). The web layer **reuses the existing transport-agnostic `kengram_mcp` orchestrators** (`search_thoughts`, `recent_thoughts`, `list_scopes`, `get_thought`, `get_related_thoughts`) and their canonical JSON mappers — so `/api/*` returns byte-identical JSON to the MCP tools rather than reimplementing retrieval. Frontend is Rust SSR (askama) + vanilla JS + a vendored `cytoscape.js` (no Node/npm/TS); static assets embedded via `rust-embed` for single-binary deploy. Four surfaces: hybrid search, thought detail w/ provenance, interactive one-hop-expand link graph, scope browser. **No schema migrations, no new MCP tools, `/mcp` wire output unchanged** — the only `kengram-mcp` delta is promoting five private response-JSON mapper fns to `pub`. v1 has no app auth: it relies on the existing localhost-bind + Caddy/Tailscale boundary (single-user, read-only); a Host-header guard mirrors the rmcp `[server].allowed_hosts` check on the new routes. Full contract in `docs/milestones/m8-human-read-surface.md`; phased progress in `docs/milestones/m8-progress.md`.
