...
- **2026-05-14** — Dogfood test-corpus pass (Claude Desktop) — 5 thoughts in scope `engram.test` probing specific extractor behaviors against `ollama/qwen3-coder:30b` v2 prompt. Six findings absorbed into M3 backlog (see `docs/milestones/m3-search-quality.md`). Three new: (1) **SPO comparative inversion** — *HIGH, silent failure*: "A is more X than B" comes back with S=B, O=A while statement is correct. 5 affected fact_ids (`8da1fa45`, `64e26652`, `fb38bf42`, `51744197`, `e0238c2f`). Any consumer keying off triple gets the inverse relationship; embedded triples will encode wrong relations once fact embeddings land. (2) **Self-referential triples** (S==O, semantically empty) — fact_ids `39016e00`, `eeced4b3`, `582b76e1`. (3) **Conditional-as-subject** — "If <cond>, A is the right choice" → S=<cond>, A in object slot — fact_ids `103f44c9`, `bea3629d`, `e9032602`. All three share the same root cause (SPO-decomposition under-specified in the prompt); all three land in v3-prompt-revision item as positive-example rules + concrete regression-test corpus. Two confirmed-open: trivia from mixed content (test 4 added new offenders `e69eff9b`, `ec465660` — single-machine benchmark results reading as durable claims) and uniform 0.85 confidence regardless of hedging (tests 2 and 3, no rubric variation). Sixth finding has nuance: today's dedup-via-supersession only covers `--rerun`; `run_reflector_once` (cron / first-time extraction) still inserts same-statement-different-triple duplicates within a single LLM call (test 4 produced `39016e00` and `bce8ac05` from the same extraction). New backlog item: extend the four-case decision tree from `run_reflector_rerun` to `run_reflector_once` via a shared `commit_or_supersede` helper. The earlier dogfood failure trio (`c5799e68` / `1c4a53c1` / `2cfe6c99` on thought `a7b63f3b`) survived the rerun cleanup because qwen3-coder produced "operations" where v1/v2 said "functions" — byte-exact statement match can't bridge that paraphrase; that's the still-deferred "Best tier" (vector-similarity) territory.

- **2026-05-14** — **M3 dedup-via-supersession landed.** The `--rerun` paraphrase-duplicate item from the morning's dogfood-driven backlog now closes its concrete failure case (v1 fact `c5799e68` / v2 fact `1c4a53c1`, identical statements but drifted triples, both parallel-active on thought `a7b63f3b`). Design principle locked with the operator: **facts table is append-only audit; supersession is the deprecation mechanism**. The dedup predicate in `find_matching_active_facts` (renamed from singular `find_matching_active_fact`, return type widened `Option<Fact>` → `Vec<Fact>`) becomes "statement match OR (S, P, O) match via IS NOT DISTINCT FROM" — either signal counts as the same claim. The reflector rerun branch becomes a three-case decision: 0 matches → insert; ≥1 match with byte-identical row → no-op floor (drift rows fold into canonical via `superseded_by`); ≥1 match without byte-identical → insert new canonical, fold drift into it. All per-claim writes in one tx. The byte-identical predicate covers (statement, S, P, O, extractor_version, confidence-within-1e-6). `superseded_by` always points at the canonical row so audit consumers walk one chain. Test count 220 → 229 (+9: storage +5, mcp/reflect +6, rename −2). The dogfood scenario is now `rerun_supersedes_when_statement_matches_but_triple_differs`. No migration; the schema was right, the dedup predicate was the bug. M3 doc updated to mark the item ✅ shipped and document what was deferred (paraphrase-similarity dedup needs fact embeddings; `--mode replace` / `--dry-run` flags out unless a real use case appears).

- **2026-05-14** — 16-hour drift observation (no new captures, just monitoring). Sharpened two M3 items, added one new one, all in `docs/milestones/m3-search-quality.md`. (1) **Re-extraction dedup**: existing item gains the operator's crude/better/best framing — crude is "supersede all existing (source_thought_id, extractor_model) facts on `--rerun` and insert fresh" (one-day fix, bleed-stopper); better is exact-statement-keyed dedup; best is vector-similarity dedup which depends on fact embeddings landing first. (2) **NEW: per-claim retraction durability** — `correct_fact` retracts a fact_id, not a claim; a subsequent `--rerun` produces new fact_ids carrying paraphrases of the retracted claim and the retraction silently doesn't propagate. Worst-failure-mode-for-a-trust-system territory. Architecturally pairs with #1: claim-keyed identity becomes both the dedup invariant *and* the retraction-durability mechanism. (3) **Confirmed via reflector_runs**: the 05:10–05:14 UTC duplicate-creating burst was manual `--rerun` invocations from our dogfood activity (Ron preferences + prompt-config sessions), **not the 3am cron**. The cron path (`run_reflector_once`/`find_unfacted_thoughts`) skips facted thoughts; only `--rerun` accumulates duplicates. Operator-side mitigation for the rest of the dogfood week: stop running `--rerun`; plain `engram reflect --limit N` is safe for new captures.

- **2026-05-13** — Second M2 dogfood pass added three more M3 backlog items, all captured in `docs/milestones/m3-search-quality.md`. From a small thought-set (two thoughts about Ron's language preferences → 6 facts): (1) **subsumption-aware dedup** — "Ron does not like Python" + "Ron does not like Python for enterprise software" both committed as active facts; a structural-subset-relation dedup is needed (distinct from the paraphrase-aware merge already on the list); (2) **structured relations in the response schema** — "Rust > Go for me; Go is the next choice when Rust isn't available" extracted as two facts that lose the explicit ranking; schema extension or prompt-flattening discussion to come; (3) **trigram should index the (S, P, O) fields, not just `statement`** — a fact whose subject="Ron" appears only in the triple (not the statement) is invisible to `search_facts("Ron …")` via the lexical leg; small storage-side fix on `search_facts_trigram`. Plus a (4): **confidence over-anchors at the v2 default 0.85** — every fact in the pass landed there, hedged or not; rolls into the v3 prompt revision already on the list. None urgent enough to interrupt dogfood; all queued for M3 planning.
