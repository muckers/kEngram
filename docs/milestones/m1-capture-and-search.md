...
Deliver a usable single-user memory service. The operator can capture thoughts via MCP and retrieve them via hybrid search. State is durable, persistent across sessions, and accessible from any MCP-capable client (Claude Code, Claude Desktop, opencode, etc.) configured to point at the same engram instance.
...
- Cargo workspace with five crates (`engram-core`, `engram-storage`, `engram-embed`, `engram-mcp`, `engram-cli`).
...
- The `Embedder` trait (in `engram-core`) plus two implementations: `TeiEmbedder` (production: HTTP client for the TEI sidecar, BGE-M3, 1024-dim, `model_id = "bge-m3:1024"`) and `CloudEmbedder` (dev/test: Voyage AI or OpenAI; off by default).
...
- Single binary `engram` with subcommands `serve` and `migrate`. No worker process.
...
- **`engram-core`** — domain types (`Thought`, `ThoughtId`, `Scope`, `Embedding`, `EmbeddingModel`, `Query`, `SearchHit`), the `Embedder` trait, RRF fusion logic. No I/O. No sqlx, no HTTP, no rmcp.
- **`engram-storage`** — sqlx-typed queries, migration runner, repository functions for `thoughts` and `embeddings` (insert thought, insert embedding, fetch thought by id, vector kNN, trigram search, recent-by-scope). Knows nothing about MCP.
- **`engram-embed`** — `TeiEmbedder` (HTTP client for the TEI sidecar) and `CloudEmbedder` (dev/test). Both implement the `Embedder` trait from `engram-core`.
- **`engram-mcp`** — `rmcp` tool descriptors and handler implementations for the four M1 tools. Depends on `engram-core`, `engram-storage`, `engram-embed`.
- **`engram-cli`** — the only binary. `main.rs`, axum bootstrap, MCP transport wiring, `figment` config loader, `tracing` init, subcommand dispatch.
...
5. **MCP smoke test:** running `engram serve` and pointing Claude Code (or `mcp-inspector`) at it, the operator can call all four tools and observe correct behavior.
...
- **Config file location.** `~/.config/engram/engram.toml`, `./config/engram.toml`, or a `--config` flag? What's the figment layer order? RJF: The first one.
