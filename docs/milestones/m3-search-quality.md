...
- **Subtractive logic + paraphrase-aware merge on `--rerun`.** M2 Phase D shipped additive-only rerun: facts the new extractor doesn't reproduce stay active. By dogfood day 2 (2026-05-14, 16-hour observation window), every manual `engram reflect --rerun` was adding 3–9 paraphrase duplicates per non-retracted thought — `"work.tcgplayer as a catch-all for work"` vs. `"work.tcgplayer as a catch-all for work-related content"` etc. Growth per-rerun, not per-day; the cron path (`run_reflector_once` via `find_unfacted_thoughts`) doesn't trigger it. Three implementation tiers, in order of completeness (operator's framing 2026-05-14):
  - **Crude (bleed-stopper):** on `--rerun` against a thought, supersede all existing active facts from the same `(source_thought_id, extractor_model)` and insert the fresh batch. Trivially correct; ignores cross-version claim continuity; gets the dups to zero without solving the deeper merge problem. One-day fix.
  - **Better:** exact-`statement`-keyed dedup within `source_thought_id`. New extraction emits "fact already present" → update confidence / extractor_version on the existing row; emits new statement → insert. Linear in fact count per thought; cheap.
  - **Best:** vector-similarity dedup on `statement` within `source_thought_id`, threshold-tunable. Collapses paraphrases across model versions and prompt revisions. Depends on fact embeddings landing first (M3 in-scope; see "Fact embeddings" above).
